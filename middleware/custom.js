...
const SchoolOtherDocs = require("../models").SchoolOtherDocs;
...
let attendance = async function (req, res, next) {
  ...
  [err, attendanceObj] = await to(
    Attendance.findOne({ where: { id: attendanceId } })
  );
  if (err) return ReE(res, "err finding attendance");

  if (!attendanceObj)
    return ReE(res, "attendance not found with id: " + attendanceId);
...
let division = async function (req, res, next) {
  ...
  [err, divisionObj] = await to(
    Division.findOne({ where: { id: divisionId } })
  );
  if (err) return ReE(res, "err finding division");

  if (!divisionObj)
    return ReE(res, "division not found with id: " + divisionId);
...
let schoolOtherDocs = async function (req, res, next) {
  let schoolOtherDocsId, err, schoolOtherDocsObj;
  schoolOtherDocsId = req.params.schoolOtherDocsId;

  [err, schoolOtherDocsObj] = await to(
    SchoolOtherDocs.findOne({ where: { id: schoolOtherDocsId } })
  );
  if (err) return ReE(res, "error finding SchoolOtherDocs");

  if (!schoolOtherDocsObj)
    return ReE(res, "SchoolOtherDocs not found with id: " + schoolOtherDocsId);

  req.schoolOtherDocs = schoolOtherDocsObj;
  next();
};
module.exports.schoolOtherDocs = schoolOtherDocs;
...
let news = async function (req, res, next) {
  let newsId, err, newsObj;
  newsId = req.params.newsId; // Make sure to use the correct parameter name from your route path

  [err, newsObj] = await to(News.findOne({ where: { id: newsId } }));
  if (err) return ReE(res, "Error finding news");

  if (!newsObj) return ReE(res, "News not found with id: " + newsId);
...
let homework = async function (req, res, next) {
  ...
  [err, homeworkObj] = await to(
    Homework.findOne({ where: { id: homeworkId } })
  );
  if (err) return ReE(res, "err finding homework");

  if (!homeworkObj)
    return ReE(res, "homework not found with id: " + homeworkId);
...
let borrowedbooks = async function (req, res, next) {
  ...
  [err, borrowedbooksObj] = await to(
    Borrowedbooks.findOne({ where: { borrowId: borrowId } })
  );
  if (err) return ReE(res, "err finding borrowedbooks");

  if (!borrowedbooksObj)
    return ReE(res, "borrowedbooks not found with id: " + borrowId);
...
let leavingcertificates = async function (req, res, next) {
  ...
  if (!leavingcertificatesObj)
    return ReE(
      res,
      "leavingcertificates not found with id: " + leavingcertificateId
    );
...
let finalresult = async function (req, res, next) {
  ...
  [err, finalresultObj] = await to(
    Finalresult.findOne({ where: { id: finalresultId } })
  );
  if (err) return ReE(res, "err finding finalresult");

  if (!finalresultObj)
    return ReE(res, "finalresult not found with id: " + finalresultId);
...
let teachersubject = async function (req, res, next) {
  ...
  [err, teachersubjectObj] = await to(
    Teachersubject.findOne({ where: { id: teachersubjectId } })
  );
  if (err) return ReE(res, "err finding teachersubject");

  if (!teachersubjectObj)
    return ReE(res, "teachersubject not found with id: " + teachersubjectId);
...
let classteacher = async function (req, res, next) {
  ...
  [err, classteacherObj] = await to(
    Classteacher.findOne({ where: { id: classteacherId } })
  );
  if (err) return ReE(res, "err finding classteacher");

  if (!classteacherObj)
    return ReE(res, "classteacher not found with id: " + classteacherId);
...
let schoolprofile = async function (req, res, next) {
  ...
  // console.log(req.params);
  console.log(req.body);
  [err, schoolprofileObj] = await to(
    Schoolprofile.findOne({ where: { id: schoolprofileId } })
  );
  if (err) return ReE(res, "err finding school profile");

  if (!schoolprofileObj)
    return ReE(res, "school profile not found with id: " + schoolprofileId);
...
let testmarks = async function (req, res, next) {
  ...
  [err, testmarksObj] = await to(
    Testmarks.findOne({ where: { id: testmarksId } })
  );
  if (err) return ReE(res, "err finding testmarks");

  if (!testmarksObj)
    return ReE(res, "testmarks not found with id: " + testmarksId);
...
let timetable = async function (req, res, next) {
  ...
  [err, timetableObj] = await to(
    Timetable.findOne({ where: { id: timetableId } })
  );
  if (err) return ReE(res, "err finding testmarks");

  if (!timetableObj)
    return ReE(res, "Timetable not found with id: " + timetableId);
