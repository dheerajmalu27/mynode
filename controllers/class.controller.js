...
const remove = async function (req, res) {
  ...
  try {
    ...
    return ReS(res, { message: "Deleted class" }, 200);
  } catch (err) {
...
const getAllListOfSubjectByClassId = async function (req, res) {
  try {
    ...
    const subjects = await db.sequelize.query(
      "SELECT s.id, s.subName as text FROM subject s JOIN class c ON FIND_IN_SET(s.id, c.subjectIds) > 0 OR FIND_IN_SET(s.id, c.optionalSubjectIds) > 0 WHERE c.id =" +
        classId,
      { type: db.sequelize.QueryTypes.SELECT }
    );
...
const getClassListForFees = async function (req, res) {
  try {
    const classlist = await db.sequelize.query(
      "SELECT `class`.`id` AS `id`,`class`.`className` AS `text` FROM `school`.`class` WHERE `class`.`id` NOT IN (SELECT DISTINCT `fees`.`classId` FROM `school`.`fees`);",
      { type: db.sequelize.QueryTypes.SELECT }
    );
