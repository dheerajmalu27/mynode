...
const create = async function(req, res){
    ...
    [err, leavingcertificatesObj] = await to(Leavingcertificate.create(leavingcertificatesInfo));
...
const get = async function(req, res){
    let leavingcertificatesId = req.params.leavingcertificatesId;
    [err, leavingcertificatesObj] = await to(Leavingcertificate.findByPk(leavingcertificatesId));
    if(err) return ReE(res, err, 422);
...
const update = async function(req, res){
    ...
    leavingcertificatesObj = req.holiday;
    data = req.body;
    console.log(data);
    ...
    return ReS(res, {message :'Updated Leavingcertificate: '+leavingcertificatesObj.id});
}
...
const getAllList = async function(req, res){
    ...
    Leavingcertificate.findAll({ attributes: ['id','studentName','className','divName','dateOfCertificate']})
        .then(att =>ReS(res, {leavingcertificates:att}))
        .catch(error => ReS(res, {leavingcertificates:error}));
}
