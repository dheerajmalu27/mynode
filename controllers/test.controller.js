...
const getAllList = async function (req, res) {
    try {
        const test = await Test.findAll({ attributes: ['id', ['testName', 'text']] });
        return ReS(res, { test });
    } catch (error) {
        return ReE(res, { test: error });
    }
...
const testClassReportList = async function (req, res) {
    try {
        ...
        const testwiseclasslist = await db.sequelize.query('CALL testwiseclasslist();');
...
const finalResultClassReportList = async function (req, res) {
    try {
        ...
        const finalResultClassList = await db.sequelize.query('SELECT * FROM finalresultclassdivisionlist');
...
const testClassDivisionReportList = async function (req, res) {
    try {
        ...
        const testdata = await db.sequelize.query(
            'SELECT * FROM testmarksview where testId=' + testId + ' and classId=' + classId + ' and divId=' + divId + ' order by rollNo, subName',
...
function calculateSubjectAverages(testResults) {
    ...
    const subjectTotals = Object.entries(subjects).map(([subject, marksArray]) => {
...
async function insertFinalResultRecords(req, res) {
    ...
    `, {
      ...
      `, {
            ...
            const { subjectTotals, subjectAverages } = calculateSubjectAverages(testResults);
